...
if (isset($_POST['editid']) && !empty($_POST['editid'])){
    $job_id = filter_input(INPUT_POST,"editid", FILTER_VALIDATE_INT);
...
//    }
    ...
    if (false !== empty($_SESSION['error'])){
        $job->UpdateJobLister($location,$contact_user, $contact_email, 2,
            $company, $job_title, $description, $salary, 1);
...
$template->job = $job->getJobDetailsById(1);
$template->category = $job->getCategory();
echo $template;
